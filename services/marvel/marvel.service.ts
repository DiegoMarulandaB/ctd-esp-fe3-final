...
interface Comic {
  ...
  oldPrice: number
  stock: number
  // Aquí podrían ir otros campos de la estructura del comic
}
...
export const getComic = async (comicId: number): Promise<Comic | null> => {
  const data: any = await fetchApi(`comics/${comicId}`)
  const results: any[] = data.data.results
  if (results.length > 0) {
    const comic: Comic = results[0]
    if (`${comic.id}`.endsWith('0')) {
      comic.price = 48
      comic.oldPrice = 48
      comic.stock = 0
...
export const getCharacters = async (): Promise<any[]> => {
  const data: any = await fetchApi('characters')
  return data.data.results
}

export const getCharacter = async (characterId: number): Promise<any | null> => {
  const data: any = await fetchApi(`characters/${characterId}`)
  const results: any[] = data.data.results
  if (results.length > 0) return results[0]
  else return null
}
...
export const getCharacterByComic = async (comicId: number): Promise<any[]> => {
  const data: any = await fetchApi(`comics/${comicId}/characters`)
  return data.data.results
}
