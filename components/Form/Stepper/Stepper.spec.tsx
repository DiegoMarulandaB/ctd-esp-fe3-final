...
describe('Stepper.spec.tsx', () => {
  describe('when rendering default', () => {
    it('should render title steps', () => {
      render(<Stepper title={''} image={''} price={0} />);
  ...
  describe('when filling form', () => {
    it('should navigate to next and previous form', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Paso 2: Dirección de entrega')).toBeInTheDocument();
      });

      userEvent.click(await screen.findByText(/anterior/i));

      await waitFor(() => {
        expect(screen.getByText('Paso 1: Datos Personales')).toBeInTheDocument();
      });
    ...
    it('should render navigate to /confirmacion-compra', async () => {
      ...
      // PASO 3
      await waitFor(() => {
        expect(screen.getByText('Paso 3: Datos del pago')).toBeInTheDocument();
      });

      await userEvent.type(screen.getByRole('textbox', { name: /Número de tarjeta */i }), '42424242 4242 4242');
      await userEvent.type(
        screen.getByRole('textbox', { name: /Nombre como aparece en la tarjeta/i }),
        'Diego Marulanda'
      );
